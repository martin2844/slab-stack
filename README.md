...
[`releases/v0.1.2-candidate.35.json`](releases/v0.1.2-candidate.35.json). Both pin
public amd64/arm64 images for all five services. Candidate.35 packages the
...
```bash
node scripts/render-image-env.mjs releases/v0.1.2-candidate.35.json
```
...
```bash
./scripts/package-release.sh releases/v0.1.2-candidate.35.json dist
```
...
```bash
sudo sh install.sh --version 0.1.2-candidate.35
```
...
```bash
sudo sh install.sh --version 0.1.2-candidate.35 -- --dry-run
```
