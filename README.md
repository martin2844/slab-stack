...
[`releases/v0.1.2-candidate.24.json`](releases/v0.1.2-candidate.24.json). Both pin
public amd64/arm64 images for all five services. Candidate.24 packages the
...
```bash
node scripts/render-image-env.mjs releases/v0.1.2-candidate.24.json
```
...
```bash
./scripts/package-release.sh releases/v0.1.2-candidate.24.json dist
```
...
```bash
sudo sh install.sh --version 0.1.2-candidate.24
```
...
```bash
sudo sh install.sh --version 0.1.2-candidate.24 -- --dry-run
```
